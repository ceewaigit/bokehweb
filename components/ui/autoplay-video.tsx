...
export const AutoplayVideo = React.forwardRef<HTMLVideoElement, AutoplayVideoProps>(
    ({
        className,
        containerClassName,
        src,
        ...
        // Sync activeSrc when prop src changes
        useEffect(() => {
            setActiveSrc(src);
        }, [src]);
        ...
        const mimeType = useMemo(() => {
            if (!activeSrc || typeof activeSrc !== 'string') return undefined;
            if (activeSrc.endsWith(".webm")) return "video/webm";
            if (activeSrc.endsWith(".mp4")) return "video/mp4";
            return undefined;
        }, [activeSrc]);

        useEffect(() => {
            // Reset states when activeSrc changes
            setIsLoading(true);
            setIsPlaying(false);
            setHasError(false);
        ...
        }, [isInView, activeSrc, hasError, videoRef]);
        ...
        const handleError = (e: React.SyntheticEvent<HTMLVideoElement, Event>) => {
            ...
            // MP4 Fallback for Decode Errors (Code 3) or Source Not Supported (Code 4)
            if ((errorCode === MediaError.MEDIA_ERR_DECODE || errorCode === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED) &&
                ...
                if (isDev) {
                    console.warn(`AutoplayVideo: WebM decode failure (${errorCode}), falling back to MP4... SRC: ${activeSrc}`);
                }
                setActiveSrc(activeSrc.replace(".webm", ".mp4"));
                return;
            }

            if (isDev) {
                console.error("AutoplayVideo Error:", errorCode, error?.message, "SRC:", activeSrc);
            }
        ...
        return (
            ...
            >
                <video
                    ref={videoRef}
                    key={String(activeSrc || 'empty')} // FORCE REMOUNT ON SOURCE CHANGE
                    ...
                    {...props}
                >
                    {typeof activeSrc === 'string' && <source src={activeSrc} type={mimeType} />}
                </video>
